...
public class ReportTable implements ReportElement {
     ...
     */
    public ReportTable(PdfConfiguration pdfConfiguration, float[] cellWidths, ReportElement[][] elements, ReportElement[] title) {
    ...
    protected ReportElement[][] getElements() {
        return elements;
    }
    ...
    private ReportTable createNewTableWithClonedSettings(ReportElement[][] data) {
        ReportTable newTable = new ReportTable(pdfConfiguration, cellWidths, data, title);
        ...
        if (i > 0) {
                ...
                if (splittable) {
                    for (int j = 0; j < elements[i].length; j++) {
                        if (elements[i][j].getHeight(cellWidths[j] * allowedWidth - cellPaddingX * 2) + currentHeight < allowedHeight) {
                            ...
                            extraRows[1][j] = new ReportTextBox(pdfConfiguration.getTextBodyConfig(), pdfConfiguration.getLineDistance(), "");
                        } else {
            ...
            for (i = 0; i < elements[0].length; i++) {
                ...
                if (splits[0] != null)
                    ...
                    first[0][i] = new ReportTextBox(pdfConfiguration.getTextBodyConfig(), pdfConfiguration.getLineDistance(), "");

                if (splits[1] != null)
                    next[0][i] = splits[1];
                else
                    next[0][i] = new ReportTextBox(pdfConfiguration.getTextBodyConfig(), pdfConfiguration.getLineDistance(), "");
            }
            ReportTable firstLine = createNewTableWithClonedSettings(first);
            ReportTable nextLines = createNewTableWithClonedSettings(next);
