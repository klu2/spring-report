...
import cc.catalysts.pdfreport.utils.PdfConfig;
...
public class ReportTable implements ReportElement {
     ...
     */
    public ReportTable(float[] cellWidths, ReportElement[][] elements, ReportElement[] title) {
    ...
    private ReportTable createNewTableWithClonedSettings(ReportElement[][] data) {
        ReportTable newTable = new ReportTable(cellWidths, data, title);
        ...
        if (i > 0) {
                ...
                if (splittable) {
                    for (int j = 0; j < elements[i].length; j++) {
                        if (elements[i][j].getHeight(cellWidths[j] * allowedWidth - cellPaddingX * 2) + currentHeight < allowedHeight) {
                            ...
                            extraRows[1][j] = new ReportTextBox(PdfConfig.getTextConfig(), "");
                        } else {
            ...
            for (i = 0; i < elements[0].length; i++) {
                ...
                if (splits[0] != null)
                    ...
                    first[0][i] = new ReportTextBox(PdfConfig.getTextConfig(), "");

                if (splits[1] != null)
                    next[0][i] = splits[1];
                else
                    next[0][i] = new ReportTextBox(PdfConfig.getTextConfig(), "");
            }
            ReportTable firstLine = createNewTableWithClonedSettings(first);
            ReportTable nextLines = createNewTableWithClonedSettings(next);
